                ...
                try {
                    environment.withMavenSettings {
                        ...
                        withEnv(mvnEnv) {
                            ...
                            def mvnCmd = "mvn -Pdebug -U -Dset.changelist help:evaluate -Dexpression=changelist -Doutput=$changelistF clean verify install ${runTests ? '-Dmaven.test.failure.ignore' : '-DskipTests'} -V -B -Dmaven.repo.local=$m2repo -s settings.xml -e"
                            sh mvnCmd
